...
try:
    ...
    en_n=np.asarray(f['energy_n'])
    pr_n=en_n*(5.0/3.0-1.0)*rho_n
    T_n=5.0/3.0*pr_n/rho_n
    axs[0,0].plot(x,rho_n[:,0,0],color='r',linestyle='--')
    axs[1,0].plot(x,vx_n[:,0,0],color='r',linestyle='--')
    axs[0,1].plot(x,pr_n[:,0,0],color='r',linestyle='--')
    axs[1,1].plot(x,T_n[:,0,0],color='r',linestyle='--')
except:
    pass
